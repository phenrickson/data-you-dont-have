...
This is a portion of the clinical data they collected on individuals from 1956 to 1968, who were then monitored for the next 24 years for various heart diseases.
...
#| class: scroll
gt_framingham_features |>
  gt::data_color(columns = c("ten_year_chd"), palette = "Reds")
...
[^4]: This is also fairly involved but can be accomplished via a ridge regression; again, I would be happy to talk about in **exhausting detail** with you later.
...
To that end, let's talk about the college football playoff
...
team_scores |>
  filter(season >= 2019) |>
  plot_team_by_week(team = 'Ohio State', ranking = c(10, 25))
```
...
Ohio State wins but doesn't cover.
